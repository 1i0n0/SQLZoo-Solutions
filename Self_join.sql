...
--    however by joining two copies of the stops table we can refer to stops 
...
-- Another solution
SELECT bus1.num, bus1.company, bus1.stop, bus2.num, bus2.company
FROM (SELECT a.num AS num, a.company AS company, stopsb.name AS stop
      FROM route a 
      JOIN route b ON (a.company = b.company AND a.num = b.num)
      JOIN stops stopsa ON a.stop = stopsa.id
      JOIN stops stopsb ON b.stop = stopsb.id 
      WHERE stopsa.name = 'Craiglockhart') AS bus1
JOIN (SELECT a.num AS num, a.company AS company, stopsa.name AS stop
      FROM route a 
      JOIN route b ON (a.company = b.company AND a.num = b.num)
      JOIN stops stopsa ON a.stop = stopsa.id
      JOIN stops stopsb ON b.stop = stopsb.id 
      WHERE stopsb.name = 'Lochend') AS bus2
ON bus1.stop = bus2.stop
ORDER BY bus1.num, stop, bus2.num;


-- ----------------------------
--  https://napier.sqlzoo.net/wiki/Self_join_Quiz
-- ----------------------------
