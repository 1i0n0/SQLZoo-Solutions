-- ----------------------------
--  https://napier.sqlzoo.net/wiki/NSS_Tutorial
-- ----------------------------
...
--3. Show the institution and score 
--    where the score for '(8) Computer Science' is less than 50 for question 'Q15'.
...
--    for each of the subjects '(8) Computer Science' and '(H) Creative Arts and Design'.
SELECT subject, 
       SUM(response * A_STRONGLY_AGREE / 100) AS num_students
FROM nss
WHERE question = 'Q22'
  AND subject IN ('(8) Computer Science', '(H) Creative Arts and Design')
GROUP BY subject;

--!!! Total number of students should be integer. !!!
--   ROUND(SUM(response * A_STRONGLY_AGREE / 100), 0) AS num_students
...
--   The column score is a percentage - you must use the method outlined above 
--    to multiply the percentage by the response and divide by the total response. 
--   Give your answer rounded to the nearest whole number.
SELECT institution, 
       ROUND(SUM(response * score / 100) / SUM(response) * 100, 0) AS avg_scores
FROM nss
WHERE question = 'Q22'
