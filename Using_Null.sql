...
--     followed by 'Sci' if the teacher is in dept 1 or 2, 
...
-- ----------------------------
--  https://napier.sqlzoo.net/wiki/Using_Null_Quiz
-- ----------------------------
...
--2.  Select the correct statement that shows the name of department 
...
--3. Select out of following the code which uses a JOIN 
--    to show a list of all the departments and number of employed teachers.
SELECT dept.name, COUNT(teacher.name) 
FROM teacher 
RIGHT JOIN dept 
ON dept.id = teacher.dept 
...
--4. Using SELECT name, dept, COALESCE(dept, 0) AS result 
--         FROM teacher on teacher table will:
/*
